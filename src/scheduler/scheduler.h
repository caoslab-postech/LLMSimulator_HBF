...
class Scheduler : public std::enable_shared_from_this<Scheduler> {
 public:
  ...
  int getUniqueTraceCount();
  void seedSteadyStateBatch();       // PP=1: running_queue based
  void seedSteadyStateMicrobatches();  // PP>1: microbatches based  // dowon revised
  int countSeededSequences();
  ...
  // dowon revised
  // --- PP decode microbatch pipeline (decode-only) ---
  // PP decode microbatches: created once, sequences stay assigned across rounds.
  // Each microbatch holds ~batch_size_per_dp/pp_dg sequences PER DP replica.
  // microbatches[microbatch_id][dp_rank] = BatchedSequence for that microbatch/dp pair.
  std::vector<std::vector<BatchedSequence::Ptr>> microbatches;

  // Create pp_dg decode microbatches from all DP replicas in running_queue.
  void initializeMicrobatches();
  ...
  // Returns total count launched across all DP replicas.
  int launchMicrobatch(int microbatch_id, const std::vector<time_ns>& stage0_launch_times);

  // Complete microbatch execution: update sequences with absolute finish time.
  // finish_abs[dp] = last stage finish for each DP replica.
  // Returns list of sequences that hit milestones.
  std::vector<Sequence::Ptr> completeMicrobatch(int microbatch_id,
      const std::vector<time_ns>& finish_abs_per_dp);

  // Refill empty microbatch positions from running_queue (new sequences from queue).
  void refillMicrobatches();
