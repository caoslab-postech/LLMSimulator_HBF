...
class ModelConfig {
 ...
 public:
  ModelConfig(int hidden_dim = 4096, int head_dim = 128, int num_layers = 32,
              ...
              int chunked_attention_window = 8192,
              int chunked_attention_full_stride = 4)
      : hidden_dim(hidden_dim),
        ...
        chunked_attention_window(chunked_attention_window),
        chunked_attention_full_stride(chunked_attention_full_stride) {
    if(q_lora_rank == 0){
  ...
  // Chunked attention: only anchor layers (idx % full_stride == 0) keep full KV cache
  bool use_chunked_attention;
  int chunked_attention_window;       // max KV tokens for non-anchor layers
  int chunked_attention_full_stride;  // every Nth layer is anchor (keeps full KV)

  int ne_tp_dg;  // non-expert tensor parallelism degree
  int e_tp_dg;   // expert tensor parallelism degree
  int pp_dg = 1;  // Pipeline parallelism degree (1 = no PP)
  std::string dataset;
...
static ModelConfig llama4_maverick = // 128 Expert
                ModelConfig(5120, 128, 48, 40, 8, 1048576, 16384, 8192, 1, 2, 128, 1, 2, 1,
                            ...
                            true, 8192, 4);  // chunked attention: anchor every 4th layer, window=8192
