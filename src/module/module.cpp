...
std::vector<long> Module::get_size() {
  ...
  for (auto _tensor : tensor_list) {
    ...
    if (tensor->tag == "act") {
      ...
      // Skip representative cache tensors (count_in_footprint == false);
      // their footprint is tracked via logical_cache_bytes_ instead.
      if (tensor->countsInFootprint()) {
        cache += tensor->getSize();
      }
    }
  }

  // Add analytically-tracked cache bytes (from representative-tensor optimization)
  cache += logical_cache_bytes_;
