...
Attention::Attention(std::string& prefix, std::string& name,
  ...
  // Set QKV projection flags: Q uses intermediate_bw, KV uses memory_write_bw
  int parallel_num = device_list.size();
  attn_qkv_proj->is_qkv_projection = true;
  attn_qkv_proj->qkv_num_heads = model_config.num_heads / parallel_num;
  attn_qkv_proj->qkv_num_kv_heads = model_config.num_kv_heads / parallel_num;
  attn_qkv_proj->qkv_head_dim = model_config.head_dim;

  add_module(attn_qkv_proj);

  // Pass decoder_idx for chunked attention KV cache sizing
  auto self_attention = SelfAttentionParallel::Create(
      ...
      model_config.compressed_kv, device_list, device, decoder_idx);
