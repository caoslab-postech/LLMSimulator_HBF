...
// Print without energy fields. Same as print() but skips print_energy().
// To restore energy output, replace printNoEnergy() calls with print().
void TimeStamp::printNoEnergy() {
  if (level != -1) {
    for (int i = 0; i < level; i++) {
      std::cout << "\t";
    }
...
// Recursive gantt printer: consistent format (no energy) for all levels.
// Called under redirected cout — does NOT open files or redirect streams.
void TimeStamp::printForGantt() {
  if (level != -1) {
    for (int i = 0; i < level; i++) {
      std::cout << "\t";
    }
    ...
    print_time();
    print_tensor();
    // print_energy() intentionally excluded for gantt format consistency
    print_util();
    std::cout << std::endl;
  }
...
void TimeStamp::exportGantt(std::string filepath, int device_id) {
  ...
  // Single file open, recursive printForGantt handles all children
  printForGantt();
