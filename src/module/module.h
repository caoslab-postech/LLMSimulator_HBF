...
class Module : public std::enable_shared_from_this<Module> {
 ...
 public:
  ...
  virtual void set_dependency_tensor(std::vector<Tensor::Ptr>& dependency_tensor_list,
                                     Tensor::Ptr tensor);

  // Non-mutating tensor lookup for cross-device dependency resolution.
  // Unlike get_activation(), does NOT overwrite shape or set ready flag.
  Tensor::Ptr get_tensor(const std::string& name) {
    auto it = tensor_list.find(name);
    if (it != tensor_list.end()) return it->second;
    fail("get_tensor: tensor not found: " + name);
    return nullptr;
  }
  ...
  // Accumulate analytical cache bytes for memory capacity checking.
  // These bytes are not backed by actual Tensor objects.
  void addLogicalCacheBytes(long long bytes) { logical_cache_bytes_ += bytes; }
  long long getLogicalCacheBytes() const { return logical_cache_bytes_; }
 ...
 protected:
  ...
  // Analytically-tracked cache footprint for representative-tensor optimization.
  // Added to the cache total in get_size() alongside physical tensor sizes.
  long long logical_cache_bytes_ = 0;
