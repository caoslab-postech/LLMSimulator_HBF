...
Tensor::Ptr ColumnParallelLinear::forward(
  ...
  // Forward QKV projection info to internal Linear for bandwidth split
  if (is_qkv_projection) {
    auto lin = std::dynamic_pointer_cast<Linear>(linear);
    lin->is_qkv_projection = is_qkv_projection;
    lin->qkv_num_heads = qkv_num_heads;
    lin->qkv_num_kv_heads = qkv_num_kv_heads;
    lin->qkv_head_dim = qkv_head_dim;
  }
