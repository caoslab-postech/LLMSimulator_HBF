...
ExecStatus Executor::executePType(LayerType layer_type,
  ...
  if (layer_type == LayerType::LINEAR) {
    exec_status =
        executeLinear(tensor_list, processor_type, use_ramulator, layer_info, device);
...
ExecStatus Executor::executeLinear(const std::vector<Tensor::Ptr>& tensor_list,
  ...
  // Dispatch to QKVLinearExecutionGPU for fused QKV projection on GPU
  if (layer_info.is_qkv_projection && processor_type == ProcessorType::GPU) {
    exec_status = QKVLinearExecutionGPU(device, input, weight, output,
                                        use_ramulator, layer_info);
  } else {
