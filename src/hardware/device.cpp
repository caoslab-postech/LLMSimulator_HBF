...
Device::Device(SystemConfig config, int device_total_rank, Cluster_ptr cluster)
  ...
  else { // for HBF simulation, we use the same dram config as B100/B200 this does not affect the performance
    dram_cfg_path = "./dram_config_HBM3E_192GB.yaml";
  }
...
void Device::run(std::vector<BatchedSequence::Ptr> sequences_metadata_list) {
  RankInfo ri = decompose_rank(device_total_rank,
      model_config.ne_tp_dg, model_config.pp_dg);
  int dp_rank = ri.dp_rank;
