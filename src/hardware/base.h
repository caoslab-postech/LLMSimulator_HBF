...
struct LayerInfo {
  ...
  // QKV projection flags: used to split Q/KV output bandwidth in fused QKV linear
  bool is_qkv_projection = false;
  int qkv_num_heads = 0;       // TP-sharded Q head count
  int qkv_num_kv_heads = 0;    // TP-sharded KV head count
  int qkv_head_dim = 0;

  // Chunked attention: non-anchor layers clamp KV cache to recent tokens only
